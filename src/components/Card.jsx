...
function Card({data,reference}) {
  return (
      ...
      <motion.div drag dragConstraints={reference} whileDrag={{scale:1.3}} className=' relative w-60 flex-shrink-0 h-72 rounded-[40px] bg-zinc-900/90 text-white px-8 py-10 overflow-hidden' > 
      <FaRegFileAlt />
      ...
      <div className='footer absolute bottom-0 w-full   left-0'>
        <div className='flex items-center justify-between px-8 py-3 mb-5'>
            ...
            <span className='w-7 h-7 bg-zinc-600 rounded-full  flex items-center justify-center'>
             { data.close?<IoCloseSharp />:<LuDownload color='#fff' size=".8em"/>}
              </span>
             </div>
